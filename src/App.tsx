...
const App: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Se Saída -> Olha o primeiro ponto. Se Entrada -> Olha o último ponto.
      const apenasCriticos = lista.filter(item => {
        const pontos = item.pontoDeParadaRelatorio || [];
        if (pontos.length === 0) return false;

        const pontoReferencia = item.sentido === 'Saída' ? pontos[0] : pontos[pontos.length - 1];
        const diferenca = converterParaMinutos(pontoReferencia?.tempoDiferenca || 0);
  ...
  if (loading) return <div style={{padding: '20px'}}>Configurando monitoramento por sentido...</div>;

  return (
    <div style={{ padding: '20px', fontFamily: 'sans-serif', backgroundColor: '#f8f9fa', minHeight: '100vh' }}>
      <h1 style={{ color: '#c0392b' }}>⚠️ Monitoramento de Atrasos Críticos (+10min)</h1>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {dadosFiltrados.map((item) => {
              ...
              // Define qual ponto usar para os dados da linha
              ...
              return (
                <tr key={item.id} style={{ borderBottom: '1px solid #eee', textAlign: 'center' }}>
                  ...
                  <td style={{ padding: '12px' }}>
                    <span style={{ fontSize: '0.9em', fontWeight: 'bold', color: item.sentido === 'Saída' ? '#e67e22' : '#2980b9' }}>
                      {item.sentido}
                    </span>
                  </td>
                  ...
                  <td style={{ padding: '12px', color: '#e74c3c', fontWeight: 'bold' }}>
                    ...
                    <div style={{ fontSize: '0.7em', color: '#999' }}>
                      (Base: {item.sentido === 'Saída' ? '1º ponto' : 'último ponto'})
                    </div>
                  </td>
                </tr>
              );
